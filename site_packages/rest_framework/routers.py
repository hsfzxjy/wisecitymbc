...
def replace_methodname(format_string, methodname):
    ...
    ret = ret.replace('{view_methodname}', methodname)
...
class SimpleRouter(BaseRouter):
    ...
    def get_routes(self, viewset):
        ...
        for methodname in dir(viewset):
            ...
            httpmethods = getattr(attr, 'bind_to_view_methods', None)
            if httpmethods:
                if methodname in known_actions:
                    raise ImproperlyConfigured('Cannot use @view_action decorator on '
                                               'method "%s" as it is an existing route' % methodname)
                httpmethods = [method.lower() for method in httpmethods]
                dynamic_view_routes.append((httpmethods, methodname))
        ...
        for route in self.routes:
            ...
            elif route.mapping == {'{httpmethod}': '{view_methodname}'}:
                for httpmethods, methodname in dynamic_view_routes:
                    initkwargs = route.initkwargs.copy()
                    initkwargs.update(getattr(viewset, methodname).kwargs)
                    ret.append(Route(
                        url=replace_methodname(route.url, methodname),
